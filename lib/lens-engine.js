...
function determineLensRoute(dream) {
  ...
  const strangeReaction = (dream.reentry?.strangeReaction || '').toLowerCase();
  const affectShift = (dream.reentry?.affectShift || '').toLowerCase();
  const smallDetail = (dream.reentry?.smallDetail || '').toLowerCase();
  const numinousImagery = /tiger|wolf|bear|owl|seal|lion|snake|dragon|white tiger|holy|enchanted|warrior|underworld|temple|giant|magical/.test(
    `${narrative} ${smallDetail}`,
  );
  const anomalousCalm = /calm|unfazed|composed|not panicked|restrained awe|alertness/.test(`${emotion} ${strangeReaction}`);
  const containedButOpen = /contained|ambiguous|unresolved/.test(ending);
  ...
  if (dream.mode === 'support' || /shame|grief|helpless|panic|terror|distress/.test(emotion) || /uncontained|crisis/.test(ending)) {
  ...
  } else if (numinousImagery && (anomalousCalm || containedButOpen)) {
    primaryLens = 'Jungian depth lens';
    secondaryLens = dream.knownPersonName ? 'Known-person inner-figure lens' : 'Affect-mismatch lens';
    confidence = 'high';
    evidence.push('Numinous imagery combined with anomalous calm or a contained but unresolved ending');
    ...
    confidence = smallDetail || affectShift ? 'high' : 'medium';
    ...
    confidence = strangeReaction || smallDetail ? 'high' : 'medium';
  ...
  if (strangeReaction) evidence.push(`Re-entry anomaly: ${dream.reentry?.strangeReaction}`);
  if (affectShift) evidence.push(`Affect shift in replay: ${dream.reentry?.affectShift}`);
  if (dream.reentry?.smallDetail) evidence.push(`Small detail recovered: ${dream.reentry.smallDetail}`);
  if (numinousImagery) evidence.push('Numinous or archetypal imagery detected');
  if (anomalousCalm) evidence.push('Affect mismatch: calm or composed stance in the presence of powerful imagery');
